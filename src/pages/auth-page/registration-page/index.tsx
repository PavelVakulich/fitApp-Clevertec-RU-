    ...
    const onFinish = useCallback(
        async (values: RegistrationFormValues) => {
            try {
                ...
                dispatch(push(PATHS.REGISTER_SUCCESS, { fromResult: true }));
            } catch (error) {
                const registerError = error as FetchBaseQueryError;
                if (registerError.status === STATUS_CODE.CONFLICT) {
                    dispatch(push(PATHS.REGISTER_ERROR_USER_EXIST, { fromResult: true }));
                    return;
                }

                dispatch(push(PATHS.REGISTER_ERROR, { fromResult: true, formValues: values }));
            }
        },
        [register, dispatch],
    );
